 ...
 */
@Injectable()
export class DepartmentFilterService implements SDSAutocompleteServiceInterface {
    ...
    getDataByText(currentrecords: number, searchValue?: string): Observable<SDSHiercarchicalServiceResult> {
        ...
        let _items: any[] = departmentData.map(departmentObj => ({ key: departmentObj.id, value: departmentObj.id + ' ' + departmentObj.name }))
        try {
            if (searchValue) {
